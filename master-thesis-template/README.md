...
This project must be built with **LuaLaTeX**, not pdfLaTeX. The template uses
`fontspec`, which requires LuaLaTeX or XeLaTeX. If your editor is set to
pdfLaTeX, compilation will stop with an error saying that `fontspec` cannot be
used with pdfTeX.

On Windows with MiKTeX, the simplest build command is:

    compile-windows.bat

This writes the output to `build/main.pdf`.
