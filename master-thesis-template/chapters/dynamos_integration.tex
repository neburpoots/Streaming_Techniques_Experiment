...
The implementation should not be a special case for one benchmark request. DYNAMOS supports multiple archetypes and multiple data providers, so a chunk-aware path must keep existing request shapes valid. The primary evaluation uses \texttt{dataThroughTtp} with one provider to isolate the large-result bottleneck. RQ3 then checks the \texttt{computeToData} archetype and a two-provider \texttt{dataThroughTtp} request, because those scenarios exercise different parts of the generated job and aggregation path.
...
\footnotesize
\begin{verbatim}
// Request metadata retained on every MicroserviceCommunication
request_metadata.correlation_id  string

// String metadata added to each chunk (streaming branch, 923b018)
stream_provider        string // data steward that produced the chunk
stream_sequence        string // per-provider sequence, starting at 1
stream_rows_processed  string // cumulative rows emitted
stream_rows_total      string // expected or final row total
stream_partial         string // "true" until the final chunk
stream_final           string // "true" on the final chunk
stream_batch_id        string // provider and sequence identifier
stream_columns         string // JSON array preserving column order
\end{verbatim}
...
\begin{table}[!htbp]
    \centering
    \footnotesize
    \caption{DYNAMOS services modified on the streaming implementation branch (commit \texttt{923b018}) relative to \texttt{main} (commit \texttt{a2d9531}). The orchestrator and policy enforcer carry the selected transport, but their policy decision and job-composition responsibilities remain unchanged.}
    ...
    \begin{tabularx}{\textwidth}{llXc}
        ...
        \texttt{orchestrator} & Go & Selects and forwards the requested transport during job composition & Yes \\
        \texttt{policy-enforcer} & Go & Propagates the transport through the existing approval response & Yes \\
        \bottomrule
    \end{tabularx}
\end{table}
