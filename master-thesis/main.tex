...
\todo{Do we need to reference sources? Original DYNAMOS papers for instance?}
...
\todo{I think I need to find a couple more studies that compare streaming techniques which are also partially used for the methodology of rq1.}
...
\todo{The nats jetstream does explicit ack? but I am pretty sure this wasn't the case or I am misunderstanding the sentence above. We need to check because I think all of the ones are set up in such a way that they are async and don't wait on acks. This was done for better speed and more equal comparison.}
...
\todo{Is it in any way possible to find more research backing into this type of testing? Do we have any papers currently in this workspace that could be cited here? Is this then novel and should we present it as such?}
...
\todo{This text here above here needs to change. The last sentence is especially bad, what does it even mean? Why are they currently not yet semantically comparable? Is this even true still? If it isn't true why isn't it true. If it isn't true what are the implication and can we just leave this out?}
...
\todo{These metrics in my opinion are not good and we have to change this. It could be done with bulletpoints with a short explanation for each metric. I think we either have to explain the metrics for each scenario seperately or do them in bulletpoints with the metric and then what the metric is in around 3 to for sentences to make sure the reader understands what it means so that they are better able to intepret the data.}
...
\todo{The text above here says something along the lines of retain the earlier completed multi-cell run. This is awful, Why would a text reference it's own runs. It should just state it did three runs, The date of the run also doesn't have to be there. Also what does the text afterwards mean, for instance why is this here: "the qualitative ranking on the clean matrix is consistent across the two runs, but absolute throughput levels differ and the reader should not directly cross-compare absolute numbers between the headline clean-matrix tables and the diagnostic scenario tables." Again this is awful for a finished thesis. Why is this the case? Is it still the case. I would rather get rid of that part. Then about the comparison scope. I think we need to quickly in two to three sentences explain that the benchmarks are run in single node, what that is, that brokers can also be run in multi-node clusters and that it can increase performance (this could have one or two sources backing the definitions and the statement) and that the multi-node runs were tested but are not part of the primary results because of the relevance to a system like dynamos and the fact that multi-node testing would extend the testing to a much larger scale}
...
\todo{If possible explain the metrics a bit. Why does nats and kafka have duplicates. Why is client-streaming so fast? Why is unary so slow? Why does unary have such low latency? Why do the brokered transports have higher latency?}
...
\todo{What is does concurrency mean in the results? Does it mean that there are multiple producer instances running in parallel?}
...
Therefore it is important to separate transport efficiency from DYNAMOS integration fit. RQ2 uses the benchmark evidence to design chunk-aware DYNAMOS data paths at the right architectural boundaries. RQ3 then validates those paths inside DYNAMOS, where startup behaviour, policy enforcement, failure handling, and real workload structure can be weighed against the isolated transport-level results reported here.
...
\todo[inline]{Example note: use this style for supervisor or self-review comments.}
...
The implementation target is deliberately different from the average-query example commonly used in DYNAMOS demonstrations. An average query is a reduction workload: it may scan many records but returns one scalar result. That tests computation near the data source, but it does not test whether DYNAMOS can transport many aligned rows. Bulk retrieval is relevant for vertical federated learning (VFL), where parties may hold complementary features for the same entities and need to exchange aligned intermediate rows, labels, embeddings, or gradients. The Scattered Directive work makes this connection explicit by extending DYNAMOS with cross-node microservice chains for policy-aware VFL workflows and by introducing Shared Execution through a Trusted Third Party as an archetype for collaborative computation \cite{jongejans2025scattered}.
...
The classic unary baseline uses the original DYNAMOS \texttt{main} branch at commit \texttt{747912e} \cite{dynamosMainCommit2026}. For the benchmark, its gRPC and message-size limits are configured high enough to complete the tested row counts. This avoids treating the historical 4 MB ceiling as the result. Classic unary remains intentionally full-buffered, because that is the behaviour the streaming implementations are meant to improve.
...
\begin{figure*}[!t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot+[mark=*] coordinates {(50000,12.240) (100000,15.939) (250000,26.880)};
...
Table~\ref{tab:dynamos-rq3-latency} and Figure~\ref{fig:dynamos-rq3-latency} show that all three chunk-aware variants reduce median completion latency compared with classic unary. At 250k rows, chunked unary is 2.21x faster than classic unary, intra-job gRPC streaming is 2.15x faster, and the RabbitMQ Streams boundary variant is 1.82x faster. The 100k row case is especially favourable to chunked unary, which finishes in 7.870s compared with 15.939s for classic unary.
...
\begin{table}[!t]
    ...
    \begin{tabular}{lrrrr}
        ...
        Chunked unary & 50k & 7.186s & 7.731s & 9 \\
        Chunked unary & 100k & 6.054s & 7.870s & 19 \\
        Chunked unary & 250k & 5.857s & 12.158s & 49 \\
        Intra-job gRPC streaming & 50k & 6.522s & 7.278s & 9 \\
        Intra-job gRPC streaming & 100k & 7.682s & 9.299s & 19 \\
        Intra-job gRPC streaming & 250k & 6.459s & 12.495s & 49 \\
        RabbitMQ Streams boundary & 50k & 6.915s & 7.809s & 9 \\
        RabbitMQ Streams boundary & 100k & 6.737s & 9.394s & 19 \\
        RabbitMQ Streams boundary & 250k & 7.014s & 14.756s & 49 \\
        \bottomrule
    \end{tabular}}
\end{table}

Table~\ref{tab:dynamos-rq3-first} shows the qualitative benefit of chunking. Classic unary cannot expose progress: the first result is the final result. The chunked variants emit partial results substantially earlier. At 250k rows, the first partial result appears after about 5.9s to 7.0s, while completion follows after 12.2s to 14.8s. This matters for workflows that can process data incrementally and for diagnosing whether a large request is still making progress.
...
\subsection{Scalability outlook}

The local benchmark is not the final distributed scalability answer. Scattered Directive extends DYNAMOS with cross-node microservice chains for policy-aware VFL workflows and uses FABRIC to automate distributed deployment \cite{jongejans2025scattered,scatteredDirectiveRepo2026}. FABRIC provides programmable compute and storage resources connected by high-speed dedicated links, which makes it a natural environment for testing DYNAMOS when the TTP and data providers are separated across sites \cite{baldin2019fabric,fabricPortal2026}.

A later FABRIC experiment should keep the RQ3 metrics from this section and add network throughput, broker lag or stream backlog, cross-site tail latency, failure recovery, concurrent-job scaling, and policy-change responsiveness. RabbitMQ Streams deserves special attention in that setting because its local overhead may be outweighed by retained-stream and replay semantics once agents are no longer co-located.
