...
The aim is not to identify a universally best mechanism, but to narrow the design space for DYNAMOS by asking which communication model best fits large, ordered, and concurrent inter-service transfer under realistic operational constraints. The completed corpus evaluates five transport mappings against the same pipeline, the same payload definitions, and the same correctness criteria, and the results are interpreted in terms of DYNAMOS integration. After the DYNAMOS implementation showed that chunked unary transfer is a realistic design option, the benchmark harness was extended with a sixth mapping, batched unary gRPC, in which one unary request carries multiple logical records. The first repetition of that extended rerun campaign completed during the concept phase and is reported in Section~\ref{sec:rq1-rerun}; the completed five-transport corpus remains the basis for the main comparison tables. The chapter body keeps the main comparison compact: the completed single-node clean matrix, three focused operational scenarios, and the resulting architectural interpretation. Full preset definitions and detailed per-case tables remain in the appendix.
...
The transport mappings were also kept as close as possible to one another. Client-streaming and per-message unary gRPC provided the direct, service-to-service reference points in the completed corpus. Kafka preserved per-flow ordering through keyed partitioning. RabbitMQ used streams rather than classic queues. NATS used JetStream with asynchronous publishing and explicit consumer acknowledgements. The brokered mappings were tuned for batched or asynchronous sending rather than for one blocking publish acknowledgement per record, so the comparison remained centered on transport behaviour rather than on intentionally slow acknowledgement patterns. The new batched-unary mapping keeps the same logical messages as the other transports, but groups them into bounded unary requests so that the sink still measures logical records rather than RPC calls.
...
The primary completed comparison in this chapter is a single-node Kubernetes benchmark across five transports: client-streaming gRPC, per-message unary gRPC, RabbitMQ Streams, NATS JetStream, and Kafka. Single-node means that the pipeline services and broker components are scheduled on one Kubernetes node, which isolates transport overhead without adding cross-node replication or placement effects. Kafka, RabbitMQ Streams, and NATS JetStream can also be deployed as multi-node broker clusters, where partitioning, replication, and placement can change both throughput and resilience \cite{kafka2026official,rabbitmq2026streams,nats2026jetstream,nuikka2021cloudnative}. Exploratory clustered broker runs were performed, but they are not used as primary evidence because they expand the experiment from DYNAMOS communication design into broker-cluster tuning. The extended rerun campaign, which adds batched-unary gRPC and a 256~KiB payload tier, is reported separately in Section~\ref{sec:rq1-rerun}.

\begin{table*}[!t]
    \centering
    \footnotesize
    \caption{Single-node synthetic-clean summary from the completed five-transport corpus, averaged across all payload sizes and concurrency levels.}
...
Table~\ref{tab:grpc-preliminary-results} shows a clear separation between the two direct gRPC mappings and the three brokered alternatives in the completed corpus. Client-streaming is the fastest transport in the clean matrix by a wide margin because it keeps one long-lived RPC open and sends many messages through that stream, avoiding the setup and response path of one unary call per message. Per-message unary is substantially slower because every message is transferred as its own request-response interaction, but the same blocking behaviour also keeps queueing low and explains its very low tail latency. This unary column is a lower-bound mapping, not the chunked unary design that is later implemented in DYNAMOS. The brokered transports form a middle band: they improve throughput over per-message unary by decoupling senders and receivers and by batching broker interaction, but they add broker persistence, routing, consumer polling, and backlog effects, which raises tail latency. Among the brokered transports, NATS JetStream achieves the highest average throughput, followed by RabbitMQ Streams and Kafka. Ordering violations are absent in the clean matrix. The duplicate counts on NATS and, to a much smaller extent, Kafka should be interpreted as at-least-once redelivery under explicit acknowledgements. The NATS consumer uses explicit acknowledgements with a configured 30~s \texttt{AckWait}; if processing or acknowledgement falls behind that window, JetStream can redeliver a message. These duplicates do not inflate the throughput number because the sink increments message throughput only for first-seen sequence numbers.

\subsubsection{Clean bulk-transfer matrix}

The size-wise breakdown in Table~\ref{tab:grpc-per-size} shows how the clean-matrix ranking changes with message size. Client-streaming remains the fastest option at every size. Its advantage over per-message unary is largest at small payloads, where per-request overhead dominates, and narrows as the payload grows. NATS JetStream is the strongest brokered transport across the four reported size bands. RabbitMQ Streams is competitive at 256~B and 1~KiB, then declines more sharply at larger payloads. Kafka stays below NATS in every size tier, but remains broadly stable and avoids the sharp collapse seen in RabbitMQ at 16~KiB.
...
All clean-matrix runs reached the target message count. Full per-case tables remain in Appendix~B, but the main pattern is already visible here: client-streaming dominates bulk transfer, per-message unary remains the low-latency reference rather than a throughput candidate, and the three brokered transports form a separate performance band with NATS at its front. The concurrency levels in this table represent parallel logical workers within one benchmark run, so they should be read as intra-run parallelism rather than as multiple independent producer deployments. Several per-case cells are visibly non-monotonic, especially for RabbitMQ Streams at 1~KiB and 16~KiB and for NATS at 256~B. Those cells are treated as motivation for the three-repetition rerun campaign rather than as stable fine-grained rankings. The robust conclusion from the current corpus is the broad separation between direct streaming, brokered transfer, and per-message unary, not the exact ordering of every broker at every concurrency.

\subsubsection{Extended rerun: batched unary and the 256~KiB tier}
\label{sec:rq1-rerun}

The extended rerun campaign adds the batched-unary mapping (100 records per unary request) and the 256~KiB payload tier that approximates the measured 273.7~KiB DYNAMOS chunk. Table~\ref{tab:grpc-rerun-matrix} reports the first completed repetition. The rerun executed on a freshly provisioned single-node cluster, so absolute throughput differs from the completed corpus (client-streaming, for example, is roughly twice as fast at 1~KiB); comparisons should be made within this table rather than across tables.

\begin{table}[t]
    \centering
    \scriptsize
    \caption{Extended rerun, first repetition: single-node synthetic-clean throughput (msg/s) by message size, averaged across concurrency 1, 4, 8, and 16. Batched unary carries 100 records per request. The NATS 256~KiB cell is concurrency~1 only; dashes mark cells that had not completed when this version was finalized.}
    \label{tab:grpc-rerun-matrix}
    \setlength{\tabcolsep}{3pt}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{lrrrrrr}
        \toprule
        Msg size & \shortstack{Stream\\msg/s} & \shortstack{Unary\\msg/s} & \shortstack{Batched\\unary msg/s} & \shortstack{RabbitMQ\\msg/s} & \shortstack{NATS\\msg/s} & \shortstack{Kafka\\msg/s} \\
        \midrule
        256 B   & 90{,}897 & 3{,}178 & 86{,}605 & 19{,}690 & 19{,}632 & 12{,}480 \\
        1 KiB   & 58{,}778 & 2{,}913 & 58{,}310 & 18{,}041 & 16{,}474 & -- \\
        4 KiB   & 23{,}324 & 2{,}883 & 33{,}107 & 9{,}005 & 10{,}964 & -- \\
        16 KiB  & 11{,}653 & 2{,}265 & 10{,}857 & 3{,}858 & 5{,}313 & -- \\
        256 KiB & 705 & 490 & 852 & 376 & 203 & -- \\
        \bottomrule
    \end{tabular}}
\end{table}

Two results matter for the thesis argument. First, batched unary closes most of the gap between per-message unary and client-streaming: at 1~KiB and concurrency~8 it reaches 61{,}184~msg/s against 55{,}478 for client-streaming and 3{,}596 for per-message unary in the same run, and it matches or exceeds client-streaming at 4~KiB and 256~KiB. The per-message unary column should therefore be read as a lower bound on request-response designs: once per-request overhead is amortised over a batch, a unary-based mapping is competitive with direct streaming. This is exactly the mechanism that the chunked-unary DYNAMOS variant exploits in RQ3. Second, at the 256~KiB tier the gRPC mappings clearly outperform the brokered transports in data throughput: batched unary moves 212.9~MiB/s and client-streaming 176.3~MiB/s, against 94.0~MiB/s for RabbitMQ Streams. The brokered band's competitiveness at small payloads does not extend to the message sizes that the DYNAMOS bulk workload actually moves.

The rerun also produced a finding of its own. The NATS JetStream mapping initially failed every 256~KiB cell: the harness had configured each stream with a 512~MiB retention cap, while one 256~KiB run moves roughly 5~GiB, so JetStream's limits-based retention silently discarded unconsumed messages and the sink could never complete. After raising the cap, the concurrency-1 cell completed at 203~msg/s; the higher-concurrency NATS cells and the remaining Kafka tiers had not completed when this version was finalized and are reported as missing data rather than as transport results. The episode is itself informative: at realistic chunk sizes, broker retention limits become an operational failure mode that does not exist for the direct gRPC mappings. The rerun further confirms the duplicate accounting described earlier: NATS again produced tens of thousands of \texttt{AckWait}-driven redeliveries in otherwise clean runs, none of which inflate the reported unique-message throughput, while all other transports completed with zero duplicates and zero ordering violations.

Scenario evidence for batched unary from the same campaign is consistent with the clean matrix. It holds the continuous 1{,}000~msg/s target (1{,}051~msg/s observed), and across three recovery repetitions it sustains 1{,}937--1{,}980~msg/s with zero post-failure duplicates, in clear contrast to the client-streaming replay behaviour in Table~\ref{tab:grpc-recovery-results}. All rerun cells are single repetitions except recovery, so these values are first evidence rather than settled characterisation.
...
The brokered transports expose three different buffering behaviours. RabbitMQ Streams resumes within 293~ms and drains the run fastest, with recovery-window throughput slightly above target, but it pays for that with 886 post-failure duplicates and a recovery-window $p_{95}$ latency near 3.9~s. Its post-recovery cell is empty because the run drains completely inside the fixed recovery window, leaving no post-window traffic to measure. NATS JetStream delivers its first post-failure message after only 2.6~ms, but the fixed recovery window still shows 7{,}847 messages of debt and no sustained recovery because the early replay plateau is followed by an extended quiet period; that quiet period is what appears as the near-zero post-window rate in the table. Kafka is the most delayed in this run: no messages arrive during the recovery window, the first post-failure message appears after about 29.2~s, and the run remains backlog-dominated until completion, although it keeps post-failure duplicates low and preserves ordering.

\subsubsection{Discussion}

The benchmark and the DYNAMOS architecture do not point to one simple answer, and that distinction matters. In the isolated pipeline benchmark, client-streaming gRPC is the strongest raw transport: it leads the clean matrix, remains stable in the continuous scenario, and avoids the broker overheads seen in the other transports. At the same time, the per-message unary result should not be read as proof that request-response transfer is unusable. It is a lower-bound mapping that pays one RPC per logical message. The DYNAMOS implementation later tests the more realistic request-response alternative: chunked unary transfer, where one unary message carries a bounded batch of rows. The extended rerun already supports that design directly: its batched-unary mapping closes most of the client-streaming versus unary gap and leads all transports at the 256~KiB tier that matches the measured DYNAMOS chunk size (Section~\ref{sec:rq1-rerun}).

DYNAMOS as implemented today has two different communication regimes. Within a deployed microservice-chain job, services communicate locally over gRPC inside a generated execution unit, where direct streaming is a natural optimisation. Across agents, DYNAMOS relies on sidecar-mediated RabbitMQ communication, ephemeral per-job queue management, and retry logic that tolerates the destination job not being ready when data is sent. The main lesson from RQ1 is therefore not that one named broker should be selected immediately. It is that the implementation should preserve chunks at the existing DYNAMOS boundaries and then decide separately whether a direct stream or a brokered stream is the better boundary mechanism.

This reframes the implementation hypotheses used in RQ2. The primary hypothesis is chunked transfer at existing boundaries, because that attacks per-message overhead and full-result buffering without discarding the DYNAMOS execution model; the rerun's batched-unary results now back this hypothesis with transport-level measurements rather than inference alone. RabbitMQ Streams is a secondary hypothesis for the distributed agent boundary. It preserves temporal decoupling, offers retained records and replay, and fits the current RabbitMQ-oriented operational model, but the local benchmark does not prove that it is faster than ordinary chunked transfer. Client-streaming gRPC remains the raw-efficiency baseline and a strong candidate for pod-local service chains, but it would need explicit reconnect, replay, and idempotence logic at DYNAMOS boundaries.
...
RQ2 therefore uses RQ1 as a design-space filter rather than as a final implementation verdict. RQ3 then validates the chunk-aware paths inside DYNAMOS, where startup behaviour, policy enforcement, failure handling, and real workload structure can be weighed against the isolated transport-level results reported here.

\subsection{Threats to Validity}

Four limits are important when interpreting these results. First, the primary five-transport comparison is based on completed single-node runs, and the focused operational scenarios are point measurements rather than confidence-interval estimates. They are strong enough for comparative interpretation, but not for a fully statistical treatment. Second, several clean-matrix cells show non-monotonic behaviour, especially in the brokered modes. The broad ranking is useful, but the exact within-broker ordering should be revisited with the three-repetition rerun campaign. Third, the completed corpus tops out at 16~KiB payloads, while a measured 5{,}000-row DYNAMOS chunk is approximately 280{,}255 bytes as a protobuf \texttt{MicroserviceCommunication}. The rerun's 256~KiB tier closes this gap for five of the six transports, but currently at a single repetition, on a freshly provisioned cluster whose absolute throughput differs from the completed corpus, and with the Kafka tiers and higher-concurrency NATS cells still incomplete. Fourth, resource measurements are supporting evidence rather than the basis for the transport ranking. They are reported only where the same sampling scope is available across compared runs, because otherwise a broker-heavy transport could appear more expensive simply because more of its infrastructure was included in the measurement.
...
Two focused scenario checks are added after the primary benchmark. The first changes the archetype to \texttt{computeToData}. The second keeps \texttt{dataThroughTtp} but uses two data providers, \texttt{UVA,VU}. These checks are included to show whether the implementation remains compatible outside the single-provider TTP path. For these concept-stage checks, the chunked variants use the completed three-repetition matrix, while the classic unary baseline was rerun with 10 repetitions after correcting the multi-provider aggregate merge.
...
\begin{table}[!t]
    \centering
    \scriptsize
    \caption{Median completion latency for DYNAMOS bulk retrieval. All cells completed 10/10 valid repetitions.}
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        Classic unary & 14.300s & 19.501s & 31.885s \\
        Chunked unary & 7.731s & 7.870s & 12.158s \\
        Intra-job gRPC streaming & 7.278s & 9.299s & 12.495s \\
        RabbitMQ Streams boundary & 7.809s & 9.394s & 14.756s \\
        \bottomrule
    \end{tabular}}
\end{table}
...
\begin{figure*}[!t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            ylabel={Median completion time (s)},
            ...
            \addplot+[mark=*] coordinates {(50000,14.300) (100000,19.501) (250000,31.885)};
            \addlegendentry{Classic unary}
            \addplot+[mark=square*] coordinates {(50000,7.731) (100000,7.870) (250000,12.158)};
            \addlegendentry{Chunked unary}
            \addplot+[mark=triangle*] coordinates {(50000,7.278) (100000,9.299) (250000,12.495)};
            \addlegendentry{Intra-job gRPC streaming}
            \addplot+[mark=diamond*] coordinates {(50000,7.809) (100000,9.394) (250000,14.756)};
            \addlegendentry{RabbitMQ Streams boundary}
        \end{axis}
    \end{tikzpicture}
    \caption{Median completion time for the local DYNAMOS bulk retrieval benchmark. Chunking is the dominant improvement: all chunk-aware variants scale better than classic unary as the returned row count grows.}
    \label{fig:dynamos-rq3-latency}
\end{figure*}

Table~\ref{tab:dynamos-rq3-latency} and Figure~\ref{fig:dynamos-rq3-latency} show that all three chunk-aware variants reduce median completion latency compared with classic unary. At 250k rows, chunked unary is 2.62x faster than classic unary, intra-job gRPC streaming is 2.55x faster, and the RabbitMQ Streams boundary variant is 2.16x faster. The 100k row case is especially favourable to chunked unary, which finishes in 7.870s compared with 19.501s for classic unary. The near-flat scaling of the chunked variants between 50k and 100k rows is explained by the fixed control-path cost analysed with Table~\ref{tab:dynamos-rq3-first}: at these sizes most of the measured time is policy handling, orchestration, and job startup rather than transfer.

The median values hide some run-to-run spread, but the conclusion remains stable at the largest row count. At 250k rows, classic unary ranged from 29.615s to 50.854s. Chunked unary ranged from 10.985s to 23.233s, intra-job gRPC streaming from 10.566s to 23.311s, and RabbitMQ Streams from 12.724s to 26.520s. The long-tail runs show why the final thesis should use repeated runs rather than single measurements, but the gap between classic unary and the chunked paths is large enough that it does not depend on one unusually fast or slow run.
...
\begin{table*}[!t]
    \centering
    \scriptsize
    \caption{Focused DYNAMOS scenario checks beyond the primary single-provider \texttt{dataThroughTtp} benchmark. The extra chunked-variant cells use three repetitions; the classic cells use the cleaned 10-repetition baseline.}
...
These scenario checks should be read as compatibility and direction-of-effect evidence rather than as the final statistical result set. They use fewer repetitions for the chunked variants than the primary benchmark. Their value for the concept thesis is that they show the implementation still works when the archetype and provider count change, and that the speedup is strongest when DYNAMOS must move large row sets through the TTP path.
...
\begin{table}[!t]
    ...
    \begin{tabular}{lrrrrr}
        ...
        \midrule
        Classic unary & 50k & 14.300s & 14.300s & 0.000s & 0 \\
        Classic unary & 100k & 19.501s & 19.501s & 0.000s & 0 \\
        Classic unary & 250k & 31.885s & 31.885s & 0.000s & 0 \\
        Chunked unary & 50k & 7.186s & 7.731s & 0.857s & 9 \\
        Chunked unary & 100k & 6.054s & 7.870s & 1.734s & 19 \\
        Chunked unary & 250k & 5.857s & 12.158s & 6.359s & 49 \\
        Intra-job gRPC streaming & 50k & 6.522s & 7.278s & 0.605s & 9 \\
        Intra-job gRPC streaming & 100k & 7.682s & 9.299s & 1.917s & 19 \\
        Intra-job gRPC streaming & 250k & 6.459s & 12.495s & 6.128s & 49 \\
        RabbitMQ Streams boundary & 50k & 6.915s & 7.809s & 0.819s & 9 \\
        RabbitMQ Streams boundary & 100k & 6.737s & 9.394s & 2.611s & 19 \\
        RabbitMQ Streams boundary & 250k & 7.014s & 14.756s & 7.861s & 49 \\
        \bottomrule
    \end{tabular}}
\end{table}

Table~\ref{tab:dynamos-rq3-first} shows the qualitative benefit of chunking. Classic unary cannot expose progress: the first result is the final result. The chunked variants emit partial results substantially earlier. At 250k rows, the first partial result appears after about 5.9s to 7.0s, while completion follows after 12.2s to 14.8s. The post-first column estimates how much of the measured time remains after the first visible chunk. It is not a pure network-transfer measurement, because the first chunk still includes policy handling, job creation, and initial SQL execution. It is nevertheless useful because it separates request startup from the observable chunk-drain phase. Read together with Table~\ref{tab:dynamos-rq3-latency}, this decomposition shows that the end-to-end ratios understate the data-path difference: every chunked variant spends roughly 6--7~s before the first chunk regardless of row count, which is the shared policy, orchestration, and job-startup cost. Using that estimate for the control path, the 250k-row comparison becomes approximately 6.4~s of chunk drain for chunked unary against roughly 26~s of post-startup time for classic unary, a difference near 4$\times$ where the end-to-end ratio reports 2.62$\times$, because both end-to-end numbers include the same fixed control-path cost. End-to-end remains the headline metric since it is what a DYNAMOS client experiences. This decomposition also matters for workflows that can process data incrementally and for diagnosing whether a large request is still making progress.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            width=\columnwidth,
            height=5.3cm,
            ylabel={Peak memory at 250k rows (MiB)},
            symbolic x coords={Classic,Chunked,gRPC,RabbitMQ},
            xtick=data,
            ...
            \addplot coordinates {(Classic,3695) (Chunked,1378) (gRPC,1495) (RabbitMQ,1715)};
        \end{axis}
    \end{tikzpicture}
    \caption{Peak sampled memory at 250k rows. Chunk-aware variants avoid the largest full-result buffer and keep memory below the classic unary baseline.}
    \label{fig:dynamos-rq3-memory}
\end{figure}

The resource data in Table~\ref{tab:dynamos-rq3-resources} and Figure~\ref{fig:dynamos-rq3-memory} support the main architectural claim. At 250k rows, classic unary reaches 3{,}695 MiB peak memory. Chunked unary stays at 1{,}378 MiB, intra-job gRPC streaming at 1{,}495 MiB, and the RabbitMQ Streams boundary variant at 1{,}715 MiB. Chunking therefore reduces memory pressure consistently.
...
Table~\ref{tab:dynamos-rq3-failure-modes} is included to avoid overstating robustness. The current benchmark supports a narrow robustness claim: chunking removes size-dependent single-message failure classes, reduces full-result memory pressure, and exposes partial progress before completion. It does not yet prove full recovery from arbitrary pod crashes, broker outages, or mid-stream policy revocation inside DYNAMOS. Those require explicit failure-injection tests in the DYNAMOS deployment, similar in spirit to the RQ1 recovery scenario.
...
RQ1 asks which streaming communication technologies, frameworks, and interaction patterns are suitable for large-scale and continuous data exchange in microservice-based systems such as DYNAMOS. At the level of isolated transport performance, client-streaming gRPC is the strongest result in the generic pipeline benchmark: it provides the highest sustained throughput on the clean matrix and the most stable pacing in the continuous scenario. That result establishes the value of direct streaming RPC as a technical baseline, while the brokered alternatives expose the cost and benefit of decoupling, persistence, and replay. The extended rerun adds an important qualification: a batched unary mapping recovers most of the streaming advantage and leads at the 256~KiB tier that matches real DYNAMOS chunks, so the decisive property for this workload is amortising per-message overhead, whether by streaming or by batching, rather than streaming itself.
...
RQ3 validates the implemented DYNAMOS variants on local bulk \texttt{dataThroughTtp} retrieval. All chunk-aware variants beat the classic unary baseline in median completion time and peak memory for 50k, 100k, and 250k row result sets. They also expose earlier partial results, which classic unary cannot do because its first result is its final result. Focused checks on \texttt{computeToData} and multi-provider \texttt{dataThroughTtp} show that the implementation is not confined to one request shape, although those checks remain concept-stage evidence. The strongest local recommendation is chunked unary. It gives the best 100k and 250k medians, the lowest 250k memory peak, and the smallest architectural shift from the original message-driven design.
...
\begin{table}[h]
    ...
    \begin{tabular}{lp{2.7cm}p{3.9cm}}
        ...
        \midrule
        \texttt{synthetic-clean} & Synthetic payloads from 256 B to 16 KiB in the completed corpus, extended to 256 KiB in the rerun harness; concurrency 1--16, no injected failure & Baseline bulk-transfer throughput, latency, and correctness matrix \\
...
\begin{table}[h]
    ...
    \begin{tabular}{lp{2cm}p{2.4cm}p{1.2cm}p{2.8cm}}
        ...
        \midrule
        Single-node clean matrix & Client-streaming, unary, RabbitMQ, NATS, Kafka & Payload 256\,B--16\,KiB, concurrency 1--16 & 1 & Full bulk-transfer baseline (Section~\ref{sec:rq1}) \\
        ...
        \midrule
        Clustered clean (3-broker) & RabbitMQ, NATS, Kafka & 4\,KiB, concurrency 8 & 3 & Clustered bulk-transfer cross-run consistency (Section~\ref{sec:rq1}) \\
